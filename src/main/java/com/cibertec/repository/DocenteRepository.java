...
import com.cibertec.entity.Docente;

public interface DocenteRepository extends JpaRepository<Docente, Integer> {

	

	@Query("select x from Docente x where (?1 is '' or x.nombre like ?1) and (?2 is '' or x.dni = ?2) and (?3 is -1 or x.ubigeo.idNivelgrado = ?3) and x.estado = ?4")       
